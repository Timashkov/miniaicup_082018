...
BaseCar::BaseCar(cpGroup * car_group, int direction, cpShape* (*foo)(cpVect, int, cpShapeFilter)) {
...
//            self.button_collision_type = car_group * 10
            mXmodification = direction == RIGHT_DIRECTION? 1: -1;
...
BaseCar::BaseCar(const BaseCar& orig) {
}

BaseCar::~BaseCar() {
}

//std::vector<cpVect> BaseCar::processed_car_body_poly(){
